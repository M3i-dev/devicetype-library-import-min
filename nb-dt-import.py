...
def createInventoryItems(inventoryitems, deviceType, nb):
    all_inventoryitems = {str(item): item for item in nb.dcim.inventory_item_templates.filter(devicetype_id=deviceType)}

    need_inventoryitems = []
    for inventoryitem in inventoryitems:
        try:
            inGet = all_inventoryitems[inventoryitem["name"]]
            print(f'Inventory Item Template Exists: {inGet.name} - {inGet.role}'
                  + f' - {inGet.device_type.id} - {inGet.id}')
        except KeyError:
            inventoryitem['device_type'] = deviceType
            need_inventoryitems.append(inventoryitem)

    if not need_inventoryitems:
        return

    all_inventory_item_roles = {str(item): item for item in nb.dcim.inventory_item_roles.filter(devicetype_id=deviceType)}
    for inventoryitem in need_inventoryitems:
        try:
            irGet = all_inventory_item_roles[inventoryitem["role"]]
            inventoryitem['role'] = irGet.id
        except KeyError:
            pass
    
    try:
        ifSuccess = nb.dcim.inventory_item_templates.create(need_inventoryitems)
        for intf in ifSuccess:
            print(f'Inventory Item Created: {intf.name} - '
              + f'{intf.role} - {intf.device_type.id} - '
              + f'{intf.id}')
            counter.update({'updated': 1})
    except pynetbox.RequestError as e:
        print(e.error)
    ...
    for deviceType in deviceTypes:
        ...
        if "device-bays" in deviceType:
            createDeviceBays(deviceType["device-bays"],
                             dt.id, nb)
        if "inventory-items" in deviceType:
            createInventoryItems(deviceType["inventory-items"],
                             dt.id, nb)
